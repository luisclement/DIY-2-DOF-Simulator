...
Latest as of 20260419:
<img width="747" height="449" alt="image" src="https://github.com/user-attachments/assets/ce430350-07de-41cc-881a-2874d71d43c3" />
...
# 2DOF Motion Rig: Stability & Protection Profile (v2.0)

## Overview
Transitioning from high-performance PID tuning to a "Hardware Protection" strategy. These 
settings are optimized to prevent IBT_2 driver failure during high-inertia events (e.g., 
wall hits in ACC) by mitigating Back-EMF voltage spikes and over-current scenarios.

---

## 1. SMC3 Firmware Settings (Protective Tune)
| Parameter | Value | Logic |
| :--- | :--- | :--- |
| **Kp** | 200 | Softens the power curve to prevent digital "snapping." |
| **Ks** | 40 | Reduces the initial mechanical "jolt" on direction change. |
| **Kd** | 25 | Maintained for stability against oscillation. |
| **PWMmax** | 140 | Capped current to protect IBT_2 from 150kg stall loads. |
| **PWMrev** | 50 | **CRITICAL:** Limits power during reversals to stop voltage spikes. |
| **Deadzone**| 2 | Keeps motors/drivers cool during idle/center. |
| **Fpwm** | 20kHz | Optimized for thermal efficiency over 35kHz silence. |

---

## 2. Hardware Protection Logic
### Transient Voltage Suppression (TVS)
* **Component:** 1.5KE33A (Unipolar TVS Diode).
* **Installation:** Shunt (Parallel) across $B+$ and $B-$.
* **Polarity:** **Silver Stripe (Cathode) MUST face Positive ($B+$)**.
* **Function:** Clamps inductive spikes at ~33V before they reach the BTS7960 chips.

### Power Buffering
* **Component:** 10,000uF Electrolytic Capacitor (Rating: 35V or 50V).
* **Installation:** Placed as close to IBT_2 input terminals as possible.
* **Function:** Absorbs Back-EMF energy generated by 150kg rig momentum.

---

## 3. Software Filtering (SimTools / ACC)
* **Smoothing:** Set to **15-20%** on Surge and Sway axes to round off telemetry spikes.
* **Main Intensity:** Capped at **75%** to ensure the Arduino never requests 100% PWM.
* **Tuning Center:** Max/Min values increased by ~50% to prevent "clipping" during crashes.

---

## 4. Hardware Health Audit (The $24V$ Rule)
* **Healthy Motor Resistance:** $2.5\Omega$ to $3.5\Omega$ (Stall current ~9.6A).
* **Damaged Motor Warning:** Resistance $< 1.5\Omega$ indicates internal shorting/melted insulation. 
* **Critical Hazard:** $0.4\Omega$ motors are incompatible with IBT_2 drivers (Stall current 60A).

---

## Technical Note: The "Wall Hit" Event
Failures during high-G crashes are caused by the motor acting as a generator. 
The combination of `PWMrev: 50` (Software), `TVS Diode` (Clamping), and `Capacitor` 
(Buffering) creates a three-stage electrical "crumple zone."
